...
class Nivel4Page extends StatefulWidget {
  final String userId;
  const Nivel4Page({super.key, required this.userId});
...
class _Nivel4PageState extends State<Nivel4Page> {
  ...
  List<String> torreA = ['verde', 'azul', 'vermelho'];
  List<String> torreB = [];
  List<String> torreC = [];

  final int tempoMaximoSegundos = 180;
  final int movimentosMaximos = 13;
  ...
  void _moverDisco(String origem, String destino) {
    if (!jogoComecou) {
      jogoComecou = true;
      // ❌ Não reiniciar tempo nem contador aqui
    }
  ...
  Future<void> _finalizarNivel() async {
    ...
    bool jogoCompleto = torreA.isEmpty &&
        torreB.isEmpty &&
        ListEquality().equals(torreC, ['verde', 'azul', 'vermelho']);

    bool dentroDoTempo = tempoTotal <= tempoMaximoSegundos;
    bool dentroDosMovimentos = movimentos <= movimentosMaximos;

    bool passou = jogoCompleto && dentroDoTempo && dentroDosMovimentos;
    ...
    await pacienteRef.collection('History_Games').add({
      ...
      'dentroDosMovimentos': dentroDosMovimentos,
      'passou': passou,
    });

    if (passou) {
      await pacienteRef.update({'nivelAtual': 5});
    }
              ...
              child: Column(
                children: [
                  ...
                  const Padding(
                    ...
                    child: Text(
                      "Leva os discos da TORRE A para a TORRE C na mesma ordem. "
                          "Planeia bem, quando achares que terminaste carrega em FINALIZAR",
                  ...
                  Container(
                    ...
                    child: Row(
                      ...
                      children: [
                        Column(
                          children: [
                            Icon(Icons.timer, color: tempoRestante <= 30 ? Colors.red : Colors.black),
